...
class Leave extends CI_Controller
{
	...
	public function Holidaysdelete($id)
    {
		$where = array('id'=>$id);
		$res = $this->Alldata->DeleteData('holiday',$where);
		if($res){
    ...
    public function EmApplication()
    {
	 ...
	 public function EmApplication1()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $emid                = $this->session->userdata('user_login_id');
            $data['employee']    = $this->employee_model->emselectByID($emid);
            $data['leavetypes']  = $this->leave_model->GetleavetypeInfo();
            $data['application'] = $this->leave_model->GetallApplication($emid);
            $this->load->view('backend/leave_apply2', $data);
        } else {
            redirect(base_url(), 'refresh');
        }
    }
    ...
    public function Add_Applications()
    {
        ...
        if ($this->session->userdata('user_login_access') != False) {
           ...
           // $appenddate   = "";
            ...
            if($type == 'Half Day') {
                $duration = $hourAmount;
				 $appenddate   = $this->input->post('startdate');
               ...
               /*  $formattedStart = new DateTime($appstartdate);
                ...
                $duration = $duration * 8; */
				 $appenddate   = $this->input->post('enddate');
				$date1=date_create($appstartdate);
				$date2=date_create($appenddate);
				$diff=date_diff($date1,$date2);
	...
	public function cancelleave($id){
		...
		$res = $this->Alldata->UpdateData('emp_leave',$data,$where);
		if($res){
			$this->session->set_flashdata('feedback', 'Successfully Cancel Leave');
		}else{
			$this->session->set_flashdata('feedback', "Somehing Wan't Wrong. Try again");
		}
		redirect(base_url("leave/EmApplication"));
	}
	
	public function leavecalendar(){
		$this->load->view('backend/leave_calendar');
	}
	
	public function holidaycalendar(){
		$this->load->view('backend/holiday_calendar');
	}
	
	public function holidaylist(){
		  $holidays = $this->leave_model->GetAllHoliInfo();
	...
	public function EmApplicationlist()
    {
        if ($this->session->userdata('user_login_access') != False) {
            $emid                = $this->session->userdata('user_login_id');
            $data['employee']    = $this->employee_model->emselectByID($emid);
            $data['leavetypes']  = $this->leave_model->GetleavetypeInfo();
            $application = $this->leave_model->GetallApplication($emid);
			
			foreach($application as $r) {

	         $data_events[] = array(
					"title"=> $r->leave_duration,
	                "start"=> $r->start_date,
	                "end"=> $r->end_date,
