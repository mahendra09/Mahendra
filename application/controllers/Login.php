...
class Login extends CI_Controller {
	...
	public function Login(){
		...
		$where=array('em_email' => $email);
		$companyid=$this->Alldata->DetailData('employee',$where);
		if(!empty($companyid)){
			$comid=$companyid[0]['em_company'];
			$comwhere=array('companyid'=>$comid);
			$data['company']=$this->Alldata->DetailData('company_register',$comwhere);
		}else{
			$data['company']="";
		}
		$this->load->view('login_next',$data);
	} 
    ...
    function validate_login($email = '', $password = '') {
        ...
        if ($query->num_rows() > 0) {
			...
			$this->session->set_userdata('company', $row->em_company);
