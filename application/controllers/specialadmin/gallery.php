...
class gallery extends CI_Controller {
	...
	public function index(){    
		if($this->session->userdata('admin_id')!=null)
		{
			...
			$this->load->view('specialadmin/gallery/index',$data);
	...
	public function edit($g_id){     
        ...
        if($this->form_validation->run() === FALSE){
			...
			$where = array('a_id'=> $a_id);
			$data1['admin']= $this->admindetail_model->get_admin($where);
			$data['gallery'] = $this->gallery_model->get_single_gallery_detail($g_id);
			$data['gallerycategory']= $this->alldata->getalldata('gallerycategory');
			$this->load->view('specialadmin/includes/header',$data1);		  
	...
	public function img_do_upload(){
        ...
        $config['img']= $pic;
		$config['upload_path'] = $_SERVER['DOCUMENT_ROOT'].'/mahakaliwebtechnology/public/upload/';
	 ...
	 public function add(){
		if($this->session->userdata('admin_id')!=null){
			...
			if($this->form_validation->run() === FALSE){
				...
				$data['gallerycategory']= $this->alldata->getalldata('gallerycategory');
				$this->load->view('specialadmin/includes/header',$data1);		  
				$this->load->view('specialadmin/gallery/add',$data);
				$this->load->view('specialadmin/includes/footer');
			}else{	
			    $data    = array('g_name' => $this->input->post('name'),
								'g_category' => $this->input->post('g_category')
	...
	public function delete($g_id){
        $res =$this->gallery_model->delete($g_id);
        if($res){
