...
class Employee extends CI_Controller {
    ...
    public function Add_employee(){
        if($this->session->userdata('user_login_access') != False) { 
			...
			$where=array('company'=>$comid);
			$data['depvalue'] = $this->Alldata->DetailData('department',$where);
			//$data['depvalue'] = $this->organization_model->depselect();
			$data['degvalue'] = $this->Alldata->DetailData('designation',$where);
			//$data['degvalue'] = $this->organization_model->desselect();
			$employeecode = $this->employee_model->getlastcode();
			
			
			$gen = substr($employeecode[0]['em_code'] ,-3 , 3  );
			...
			$data['newemployeecode'] = $employeecodename.'_'.str_pad($gen + 1, 3, 0, STR_PAD_LEFT);
    ...
    public function view(){
		...
		$empid = $this->Alldata->DetailData('employee',$where);
		...
		$data['assignemp'] = $this->Alldata->assignempdetail($empid[0]['id']);
      ...
      public function view1(){
        ...
        $data['socialmedia'] = $this->employee_model->GetSocialValue($id);
            $year = date('Y');
        $data['Leaveinfo'] = $this->employee_model->GetLeaveiNfo($id,$year);
        $this->load->view('backend/employee_view1',$data);
        }
    else{
        redirect(base_url() , 'refresh');
    }         
    ...
    public function Disciplinary(){
        ...
        $data['allemployees'] = $this->employee_model->getemployeetype();
	...
	public function activeemployeeactions(){
		$id = base64_decode($this->input->get('I'));
		$where = array('em_id'=>$id);
		$data = array('status'=>'ACTIVE');
		$res = $this->Alldata->UpdateData('employee',$data,$where);
		
		if($res){
			$this->session->set_flashdata('feedback','Successfully ACTIVE Employee');
            redirect(base_url().'employee/Inactive_Employee' , 'refresh');
		}else{
			$this->session->set_flashdata('feedback','Successfully Not  ACTIVE Employee');
            redirect(base_url().'employee/Inactive_Employee' , 'refresh');
		}
	}
	
	public function employeesdirectory(){
		$data['employee'] = $this->Alldata->allemployeedetail1();
		$this->load->view('backend/employeesdirectory',$data);
	}
	
	public function employeesreporter(){
		
		$where=array('em_company'=>$this->session->userdata('company'));
		$data['employee'] = $this->Alldata->DetailData('employee',$where);
		
		$where=array('company'=>$this->session->userdata('company'));
		$data['Assignemp'] = $this->Alldata->DetailData('reportingemp',$where);
		$fromemp=array();
		if(!empty($data['Assignemp'])){
			foreach($data['Assignemp'] as $ingnoremp){
				$fromemp[]=$ingnoremp['reportingempfrom'];
			}
		}
		
		
		$data['employeefrom'] = $this->Alldata->withoutassignemp($fromemp);
		
		$this->load->view('backend/employeesreporter',$data);
	}
	
	public function Save_employeesreporter(){
		if($this->session->userdata('user_login_access') != False) { 
			$to = $this->input->post('to');
			$from = $this->input->post('from');
			$this->load->library('form_validation');
			$this->form_validation->set_error_delimiters('<div class="error">', '</div>');
			$this->form_validation->set_rules('to','Employee Report To','trim|required|xss_clean');
			$this->form_validation->set_rules('from','Employee Report From','trim|required|xss_clean');
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data = array(
								'reportingempto' => $to,
								'reportingempfrom' => $from,
								'company' => $comid
							);
				$success = $this->Alldata->insertData('reportingemp',$data);
				$this->session->set_flashdata('feedback','Successfully Added');
				
				redirect(base_url().'employee/employeesreporter' , 'refresh');
			}
		}
		else{
			redirect(base_url() , 'refresh');
	...
	public function employeesreporter_edit($id){
        if($this->session->userdata('user_login_access') != False) { 
			$where=array('em_company'=>$this->session->userdata('company'));
			$data['employee'] = $this->Alldata->DetailData('employee',$where);
			$data['employeefrom'] = $this->Alldata->DetailData('employee',$where);
			$where=array('company'=>$this->session->userdata('company'));
			$data['Assignemp'] = $this->Alldata->DetailData('reportingemp',$where);
			
			$where=array('reportingempid'=>$id);
			$data['edit'] = $this->Alldata->DetailData('reportingemp',$where);
	...
	public function employeesreporter_update(){
		if($this->session->userdata('user_login_access') != False) { 
			$to = $this->input->post('to');
			$from = $this->input->post('from');
			$id = $this->input->post('id');
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$where = array('reportingempid'=>$id);
				$success = $this->Alldata->UpdateData('reportingemp',$data,$where);
	...
	public function Delete_assignemp($id){
        if($this->session->userdata('user_login_access') != False) { 
			$where = array('reportingempid'=>$id);
			$this->Alldata->DeleteData('reportingemp',$where);
