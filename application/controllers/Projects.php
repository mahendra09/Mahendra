...
class Projects extends CI_Controller
{
		...
		if ($this->session->userdata('user_login_access') != False) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if (empty($id)) {
					...
					//echo "Successfully Added";
					$this->session->set_flashdata('feedback','Successfully Add');
					redirect(base_url().'Projects/All_Tasks' , 'refresh');
				} /*else {
	...
	public function Project_issue(){
		...
		$data['ticket'] = $this->Alldata->DetailData('issue',$where,$orderby);
		
		$this->load->view('backend/issue_view', $data);
	}
	...
	public function Add_Ticket(){
		...
		$where=array('company'=>$comid);
		$orderby= 'issueid DESC';
		$lastticketid = $this->Alldata->DetailData('issue',$where,$orderby);
		if(isset($lastticketid) && !empty($lastticketid)){
			$ticketid = $lastticketid[0]['ticketid']+1;
		}else{
			$ticketid = 1;
		}
		$data = array(
		...
		if(!empty($this->input->post('assignto'))){
			$data['assignto']=$this->input->post('assignto');
			$data['assigndate']=date('Y-m-d');
		}
		if(isset($_FILES['screenshort']['name']) && !empty($_FILES['screenshort']['name'])) 
		{
			$imgfilename =  $this->img_do_upload();
			$data['screenshort'] = $imgfilename['upload_data']['file_name'];
		}
		$res=$this->Alldata->insertData('issue',$data);
		if($res){
			$this->session->set_flashdata('feedback','Successfully Added');
			
		}else{
			$this->session->set_flashdata('feedback','Something Want Wrong. Please Try Again...');
		}
		redirect("projects/Project_issue");
	}
	
	public function img_do_upload(){
        if(!empty($_FILES['screenshort']['name'])){
            $ext = pathinfo($_FILES['img']['name']);
			$pic= uniqid().'.'.$ext['extension']; 	 
        }
		
        $config['screenshort']= $pic;
		$config['upload_path'] = $_SERVER['DOCUMENT_ROOT'].'/hr/public/screenshort/';
