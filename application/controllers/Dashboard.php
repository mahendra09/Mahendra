...
class Dashboard extends CI_Controller {
    ...
    function Dashboard(){
        if($this->session->userdata('user_login_access') != False) {
            ...
            $profilecheck = $this->employee_model->GetBasic($id);
			
			//$data['birthday'] = $this->Alldata->birthday();
			$data['birthday'] = '';
			$data['anniversaries'] = '';
			$where=array('company'=>$comid);
			$orderby="'id','DESC'";
			$data['holidaylist'] = $this->Alldata->DetailData('holiday',$where,$orderby);
			
			if($profilecheck->des_id == ""){
				$url = base_url().'employee/view?I='.base64_encode($profilecheck->em_id);
